...
for (const path of ['/', '/privacy/', '/terms/']) {
  const context = await browser.newContext({ viewport: { width: path === '/' ? 390 : 1280, height: 844 } });
  ...
  console.log(JSON.stringify({ path, basics, consoleErrors, serious: serious.map(({ id, impact, nodes }) => ({ id, impact, nodes: nodes.length })) }));
  if (path === '/') await page.screenshot({ path: '/tmp/flaky-test-casefile-mobile.png', fullPage: true });
  await context.close();
}
await browser.close();
